...
namespace UCTS.Manager.BL
{
    public class TravelFactory : ITravelFactory
    {
        ...
        {
            var rndGen = new Random(Guid.NewGuid().GetHashCode());
            var carConstraints = dictCarConstraints[carType];
            int numberOfPassengers = 1 + rndGen.Next(carConstraints.NumberOfSeats);
            double travelingDistance = Math.Round(MIN_TRAVELING_DISTANCE + rndGen.NextDouble() * (MAX_TRAVELING_DISTANCE - MIN_TRAVELING_DISTANCE), 1); //from 1.0 to <50
